...
class SceneManager(object):
    ...
    def _process_frame(self, frame_num, frame_im, base_timecode):
        ...
        for detector in self._detector_list:
            cut_output = detector.process_frame(frame_num, frame_im)
            if len(cut_output) > 0:
                if len(cut_output) > 1:
                    raise Exception('Multiple cuts are not supported ' + ','.join(str(FrameTimecode(cut, base_timecode).get_seconds()) for cut in cut_output))

                # cut output list is 0 or 1
                if len(self._cutting_list) == 0:
                    start_scene = str(FrameTimecode(0, base_timecode).get_seconds()) # need start time hack
                else:
                    start_scene = str(FrameTimecode(self._cutting_list[-1], base_timecode).get_seconds())
                # print ('%cut:' + ','.join(str(FrameTimecode(cut, base_timecode)) for cut in cut_output))
                # print ('timecode ' + str(base_timecode) + ' ' + str(cut_output[0]))
                print ('%cut:' + start_scene + '-' + str(FrameTimecode(cut_output[0], base_timecode).get_seconds()))
            self._cutting_list += cut_output
        for detector in self._sparse_detector_list:
            event_output = detector.process_frame(frame_num, frame_im)
            self._event_list += event_output
            if len(event_output) > 0:
                # print ('%event: ' + ','.join(str(FrameTimecode(cut, base_timecode)) for cut in event_output))
                raise Exception('Event output not implemented')

        # TODO for get next func
        # if len(event_output) > 0 or len(cut_output) > 0:
        #    input()
    ...
    def detect_scenes(self, frame_source, end_time=None, frame_skip=0,
                      show_progress=True, base_timecode=0):
        ...
        try:
            ...
            while True:
                ...
                if not ret_val:
                    break
                self._process_frame(self._num_frames + start_frame, frame_im, base_timecode)
            ...
            # post scene last to end
            # no scenes detected
            if len(self._cutting_list) == 0:
                print ('%cut:' + str(FrameTimecode(0, base_timecode).get_seconds()) + '-' + str(FrameTimecode(curr_frame, base_timecode).get_seconds()))
            else:
                start_scene = str(FrameTimecode(self._cutting_list[-1], base_timecode).get_seconds())
                print ('%cut:' + start_scene + '-' + str(FrameTimecode(curr_frame, base_timecode).get_seconds()))
